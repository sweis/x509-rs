...
//!
//! Test certificates are in testdata/certs/ (generated by testdata/certs/generate.sh).
//! Reference outputs from openssl x509 are in testdata/certs/reference/.
//!
...
fn load_cert(name: &str) -> Vec<u8> {
    std::fs::read(cert_path(name)).unwrap_or_else(|e| {
        panic!(
            "Failed to read test certificate '{}': {}. Run testdata/certs/generate.sh first.",
            name, e
        )
    })
}
...
mod degenerate {
    ...
    /// Helper: build a path under testdata/certs/degenerate/
...
// testdata/certs/reference/ for regression detection.
...
// testdata/x509-limbo/). Reads the upstream limbo.json directly and filters
// at runtime. Tests skip gracefully if the submodule is not initialized.
//
// To initialize:  git submodule update --init testdata/x509-limbo
// To update:      git submodule update --remote testdata/x509-limbo
                ...
                None => {
                    eprintln!(
                        "skipping limbo tests: submodule not initialized \
                         (run: git submodule update --init testdata/x509-limbo)"
                    );
                    return;
                }
            }
